    ...
    return (
      <>
        ...
        <main className="mx-6 mt-[33px]">
          <button
            className="-tarcking-[0.25px] h-[15px] w-20 bg-[url(/assets/icon-arrow-left.svg)] bg-[left_top_1px] bg-no-repeat text-right text-[15px]/[1] font-bold text-8"
            onClick={() => navigate("/")}
          >
            Go back
          </button>
          <div className="mt-[31px] flex items-center justify-between rounded-lg bg-white p-6 shadow-[0_10px_10px_-10px_rgba(72,84,159,0.1)]">
            <span className="text-[13px]/[15px] font-normal -tracking-[0.1px] text-6">
              Status
            </span>
            <Status status={invoice.status as TStatus} />
          </div>
          <section className="mt-4 rounded-lg bg-white p-6 text-[13px]/[18px] -tracking-[0.1px] text-7 shadow-[0_10px_10px_-10px_rgba(72,84,159,0.1)]">
            <div>
              <h2 className="text-[15px]/[1] font-bold -tracking-[0.25px] text-8">
            ...
            <div className="mt-[30px] grid  grid-cols-2">
              <div className="flex flex-col">
                ...
                <span className="mt-[13px] text-[15px]/[20px] font-bold -tracking-[0.25px] text-8">
                  {invoiceDate?.slice(4, 16)}
                </span>
              </div>
              <div>
                <span className="flex flex-col gap-[13px]">Payment Due</span>
                <span className="mt-[7px] block text-[15px]/[20px] font-bold -tracking-[0.25px] text-8">
            ...
            <div className="flex flex-col">
              <span>Sent to</span>
              <span className="mt-[10px] text-[15px]/[20px] font-bold -tracking-[0.25px] text-8">
                {invoice.clientEmail}
              </span>
            </div>
            <div>
              <div className="mt-[38px] flex flex-col gap-6 rounded-t-lg bg-[#F9FAFE] p-6">
                {invoice.items.map((item) => {
                  return (
                    ...
                    >
                      <div>
                        <span className="text-[15px]/[15px] font-bold -tracking-[0.25px] text-8">
                          {item.name}
                        </span>
                        <div className="mt-2">
              ...
              <div className="flex items-center justify-between rounded-b-lg bg-[#373B53] p-6 text-white">
        ...
        <footer className="mt-14 flex justify-between gap-2 bg-white p-6">
          <button className="rounded-3xl bg-[#F9FAFE] px-6 py-4 text-[15px]/[1] font-bold -tracking-[0.25px] text-7">
            Edit
          </button>
